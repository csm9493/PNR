...
class MoCoV2Plus(BaseMomentumModel):
    ...
    def __init__(
        self, output_dim: int, proj_hidden_dim: int, temperature: float, queue_size: int, pnr_loss: bool=False, **kwargs
    ):
    ...
    def add_model_specific_args(parent_parser: argparse.ArgumentParser) -> argparse.ArgumentParser:
        ...
        parser.add_argument("--pnr_loss", action='store_true')
    ...
    def training_step(self, batch: Sequence[Any], batch_idx: int) -> torch.Tensor:
        ...
        if self.pnr_loss:
            out.update({"loss": out["loss"], "z": [q1, q2], "k": [k1, k2], "feats": [feats1, feats2], 'queue': queue})
        else:
            out.update({"loss": out["loss"] + nce_loss, "z": [q1, q2], "k": [k1, k2], "feats": [feats1, feats2], 'queue': queue})
