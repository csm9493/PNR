...
def vicreg_pnr_wrapper(Method=object):
    class VICReg_PNR_Wrapper(base_distill_wrapper(Method)):
        ...
        def training_step(self, batch: Sequence[Any], batch_idx: int) -> torch.Tensor:
            ...
            negative_loss = (invariance_loss(p1, frozen_z2) + invariance_loss(p2, frozen_z1)) / 2

            self.log("train_predictive_distill_loss", distill_loss, on_epoch=True, sync_dist=True)
            self.log("train_predictive_negative_loss", negative_loss, on_epoch=True, sync_dist=True)

            return out["loss"] + self.distill_lamb * distill_loss - self.negative_lamb * negative_loss

    return VICReg_PNR_Wrapper
