...
def barlow_pnr_wrapper(Method=object):
    class Barlow_PNR_Wrapper(base_distill_wrapper(Method)):
        def __init__(
            self,
            distill_lamb: float,
        ...
        def training_step(self, batch: Sequence[Any], batch_idx: int) -> torch.Tensor:
            ...
            negative_loss = (simsiam_loss_func(p1, frozen_z2) + simsiam_loss_func(p2, frozen_z1)) / 2

            self.log(
                "train_decorrelative_distill_loss", distill_loss, on_epoch=True, sync_dist=True
            )
            self.log(
                "train_negative_loss", negative_loss, on_epoch=True, sync_dist=True
            )

            return out["loss"] + self.distill_lamb * distill_loss - self.negative_lamb*negative_loss

    return Barlow_PNR_Wrapper
