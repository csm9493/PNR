...
def predictive_distill_wrapper(Method=object):
    class PredictiveDistillWrapper(base_distill_wrapper(Method)):
        def __init__(self, distill_lamb: float, distill_proj_hidden_dim, distill_type, **kwargs):
        ...
        def add_model_specific_args(
            ...
            parser.add_argument("--distill_type", type=str, default='byol')
        ...
        def training_step(self, batch: Sequence[Any], batch_idx: int) -> torch.Tensor:
            ...
            if self.distill_type == 'byol':
                distill_loss = (byol_loss_func(p1, frozen_z1) + byol_loss_func(p2, frozen_z2)) / 2
            else:
                distill_loss = (simsiam_loss_func(p1, frozen_z1) + simsiam_loss_func(p2, frozen_z2)) / 2
